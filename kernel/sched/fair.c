...
RB_DECLARE_CALLBACKS(static, min_vruntime_cb, struct sched_entity, run_node,
		     min_vruntime, min_vruntime_update);
...
static struct sched_entity *pick_eevdf(struct cfs_rq *cfs_rq)
{
	...
	while (node) {
		...
		if (left && vruntime_eligible(
				    cfs_rq, __node_2_se(left)->min_vruntime)) {
			node = left;
			continue;
		}
					    ...
					    unsigned int ref_th)
{
	...
	if (now - start > th_period &&
		...
		ref_cand = rate_limit * sysctl_numa_balancing_scan_period_max /
			   MSEC_PER_SEC;
		nr_cand = node_page_state(pgdat, PGPROMOTE_CANDIDATE);
...
static bool vma_is_accessed(struct mm_struct *mm, struct vm_area_struct *vma)
{
	 ...
	 */
	if ((READ_ONCE(current->mm->numa_scan_seq) -
	     vma->numab_state->start_scan_seq) < 2)
		return true;

	pids = vma->numab_state->pids_active[0] |
	       vma->numab_state->pids_active[1];
...
retry_pids:
	...
	do {
		...
		if (!vma->numab_state) {
			vma->numab_state = kzalloc(
				sizeof(struct vma_numab_state), GFP_KERNEL);
			if (!vma->numab_state)
				continue;
		 ...
		 */
		if (mm->numa_scan_seq &&
		    time_before(jiffies, vma->numab_state->next_scan)) {
			trace_sched_skip_vma_numa(mm, vma,
		...
		if (mm->numa_scan_seq &&
			...
			vma->numab_state->pids_active_reset =
				vma->numab_state->pids_active_reset +
				msecs_to_jiffies(VMA_PID_RESET_PERIOD);
			vma->numab_state->pids_active[0] =
				READ_ONCE(vma->numab_state->pids_active[1]);
...
static inline bool load_avg_is_decayed(struct sched_avg *sa)
{
	 ...
	 * break this.
	 */
	SCHED_WARN_ON(sa->load_avg || sa->util_avg || sa->runnable_avg);
...
static inline int update_cfs_rq_load_avg(u64 now, struct cfs_rq *cfs_rq)
{
	...
	u64_u32_store_copy(sa->last_update_time, cfs_rq->last_update_time_copy,
			   sa->last_update_time);
	return decayed;
}
...
static inline int util_fits_cpu(unsigned long util, unsigned long uclamp_min,
				unsigned long uclamp_max, int cpu)
{
...
/*
* OS_PROJECT: here the next entity is selected based on minimal vruntime
*/
...
static inline void sched_fair_update_stop_tick(struct rq *rq,
					       struct task_struct *p)
{
}
#endif
			   ...
			   bool has_idle_core, int target)
{
	...
	if (static_branch_unlikely(&sched_cluster_active)) {
		...
		if (sg->flags & SD_CLUSTER) {
			for_each_cpu_wrap(cpu, sched_group_span(sg),
					  target + 1) {
				if (!cpumask_test_cpu(cpu, cpus))
					continue;

				if (has_idle_core) {
					i = select_idle_core(p, cpu, cpus,
							     &idle_cpu);
...
static inline bool asym_fits_cpu(unsigned long util, unsigned long util_min,
				 unsigned long util_max, int cpu)
{
	if (sched_asym_cpucap_active())
		/*
 ...
 * exceed @eenv->cpu_cap.
 */
static inline unsigned long eenv_pd_max_util(struct energy_env *eenv,
					     struct cpumask *pd_cpus,
 ...
 */
static inline unsigned long compute_energy(struct energy_env *eenv,
...
static int find_energy_efficient_cpu(struct task_struct *p, int prev_cpu)
{
	...
	unsigned long p_util_min =
		uclamp_is_used() ? uclamp_eff_value(p, UCLAMP_MIN) : 0;
	unsigned long p_util_max =
		uclamp_is_used() ? uclamp_eff_value(p, UCLAMP_MAX) : 1024;
	...
	for (; pd; pd = pd->next) {
		...
		if (prev_spare_cap > -1) {
			prev_delta =
				compute_energy(&eenv, pd, cpus, p, prev_cpu);
			       ...
			       struct sched_group *group)
{
	 ...
	 * will not be on.
	 */
	if (group->flags & SD_SHARE_CPUCAPACITY && sgs->sum_h_nr_running > 1)
		return true;
					  ...
					  struct task_struct *p)
{
	...
	for_each_cpu(i, sched_group_span(group)) {
		...
		if (sd->flags & SD_ASYM_CPUCAPACITY &&
		    sgs->group_misfit_task_load && task_fits_cpu(p, i))
	...
	case group_has_spare:
		...
		if (sd->flags & SD_NUMA) {
			...
			if (p->nr_cpus_allowed != NR_CPUS) {
				struct cpumask *cpus = this_cpu_cpumask_var_ptr(
					select_rq_mask);

				cpumask_and(cpus, sched_group_span(local),
					    p->cpus_ptr);
				imb_numa_nr = min(cpumask_weight(cpus),
						  sd->imb_numa_nr);
			}

			imbalance =
				abs(local_sgs.idle_cpus - idlest_sgs.idle_cpus);
...
next_group:
	...
	if (sds->busiest)
		sds->prefer_sibling =
			!!(sds->busiest->flags & SD_PREFER_SIBLING);
...
__init void init_sched_fair_class(void)
{
	...
	for_each_possible_cpu(i) {
		...
		zalloc_cpumask_var_node(&per_cpu(select_rq_mask, i), GFP_KERNEL,
					cpu_to_node(i));
		zalloc_cpumask_var_node(&per_cpu(should_we_balance_tmpmask, i),
					GFP_KERNEL, cpu_to_node(i));

#ifdef CONFIG_CFS_BANDWIDTH
		INIT_CSD(&cpu_rq(i)->cfsb_csd, __cfsb_csd_unthrottle,
			 cpu_rq(i));
